...
import org.xiao.rss.Feed;
import org.xiao.rss.Location;
import org.xiao.rss.Media;
...
public class Item {
    ...
    private org.xiao.rss.Location location;
    private java.util.ArrayList<org.xiao.rss.Media> media = new java.util.ArrayList<org.xiao.rss.Media>();
    ...
    protected Item(Node item, java.util.HashMap<String, String> namespaces) {
        ...
        for (int i=0; i<nodeList.getLength(); i++){
            ...
            if (node.getNodeType()==1){
                ...
                String nodeValue = org.xiao.rss.Parser.getNodeValue(node).trim();
                if (nodeValue.length()==0) nodeValue = null;

              //Parse Common Attributes
                if (nodeName.equals("title")) title = nodeValue;
                else if (nodeName.equals("guid")) guid = nodeValue;
              ...
              //Parse Link
                    ...
                    if (url.length()==0){
                      //get href attribute
                        url = org.xiao.rss.Parser.getAttributeValue(node,"href").trim();
                    }
                    if (url.length()>0){
              ...
              //Enclosure (e.g. TASS News agency)
                    ...
                    addMedia(new org.xiao.rss.Media(node));
                }
                else if (nodeName.equals("category")){
                    category = nodeValue;
                }

                else if(org.xiao.rss.Location.isLocationNode(nodeName, namespaces)){
                    location = new org.xiao.rss.Location(node, namespaces);
                }
      ...
      //Parse media nodes
        if (!mediaNodes.isEmpty()){
            ...
            for (Node mediaNode : mediaNodes){
                ...
                if (nodeName.equals(mediaNS + ":content")){
                    ...
                    for (int i=0; i<nodes.getLength(); i++){
                        ...
                        if (node.getNodeType()==1){
                            addMedia(new org.xiao.rss.Media(mediaNode));
                            break;
                        }
                    }
          ...
          //If none of the of the content nodes have children (standard use case)
            if (media.isEmpty()){
                addMedia(new org.xiao.rss.Media(mediaNodes.toArray(new Node[mediaNodes.size()])));
            }
        }
      ...
      //Set location
        if (lat!=null && lon!=null){
            location = new org.xiao.rss.Location(lat, lon);
        }
    }
  ...
  //** addMedia
  //**************************************************************************
    public void addMedia(org.xiao.rss.Media media){
   ...
   */
    public org.xiao.rss.Media[] getMedia(){
   ...
   *  GeoRSS element).
   */
    public org.xiao.rss.Location getLocation(){
        return location;
    }
